...
class ExtendedTimeStepWrapper(dm_env.Environment):
    ...
    def _augment_time_step(self, time_step, action=None):
        ...
        try:
            obj_pos = self._env.physics.named.data.site_xpos['object_site'].copy()
        except KeyError:
            try:
                # Some specific tasks like bowl_stack_dex use 'bowl_top_site'
                obj_pos = self._env.physics.named.data.site_xpos['bowl_top_site'].copy()
            except KeyError:
                obj_pos = np.zeros(3, dtype=np.float32) # Fallback if no object exists
        return ExtendedTimeStep(observation=time_step.observation,
                                step_type=time_step.step_type,
                                action=action,
                                reward=time_step.reward or 0.0,
                                discount=time_step.discount or 1.0,
                                obj_pos=np.float32(obj_pos))
